...
async def run_alpaca_ws_proxy(symbol: str, message_handler: Callable[[str], None], stop_event: asyncio.Event):
    ...
    # Track state for trade classification
        ...
        async with websockets.connect(socket_url) as ws:
            ...
            while not stop_event.is_set():
                try:
                    ...
                    message_handler(json.dumps({"error": "Alpaca stream closed. Reconnecting..."}))
                    break
    except Exception as e:
        logger.error(f"Alpaca WebSocket proxy run failed: {e}")
